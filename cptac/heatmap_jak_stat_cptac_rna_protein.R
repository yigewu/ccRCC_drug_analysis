...
row_fontsize <- 9
## set genes to plot
load(file = "./Ding_Lab/Projects_Current/PanCan_Phospho-signaling/Resources/Gene_Lists/2015-08-01_Gene_Set.RData")
genes2plot <- KEGG[["hsa04630	Jak-STAT signaling pathway"]]
...
p <- Heatmap(mat2plot[(rowSums(!is.na(mat2plot)) >= num_row_nonna),],
             column_title = "JAK-STAT Pathway mRNA Expressin in CPTAC ccRCC Cohort",
             col = col_rna,
             name = "log10(RPKM+1)", 
...
file2write <- paste0(dir_out, "JAK_STAT_mRNA_Expression_log10RPKM.", run_id, ".png")
...
## plot heatmaps
p <- Heatmap(mat2plot[(rowSums(!is.na(mat2plot)) >= num_row_nonna),],
             column_title = "JAK-STAT Pathway Protein Expressin in CPTAC ccRCC Cohort",
             col = col_protein,
             name = "log2Intensity\n(Sample-Reference)", 
             row_names_gp = gpar(fontsize = 9),
             top_annotation = ca,
             ...
             cluster_rows = F)
## save plot for RNA expression
file2write <- paste0(dir_out, "JAK_STAT_Protein_Expression_log10RPKM.", run_id, ".png")
