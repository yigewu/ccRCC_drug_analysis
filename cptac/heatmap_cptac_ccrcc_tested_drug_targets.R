...
# protein_test_df <- fread(data.table = F, input = "../ccRCC_snRNA/Resources/Analysis_Results/bulk/expression/protein/compare_bulk_protein_tumor_vs_normal/diff_expr_proteins_tumor_vs_normal.20210506.csv")
...
proteinids_plot_df <- data.frame(Gene = c("MET", "MET", "AXL", "FLT1", "KDR", "RET", "KIT", "TIE2",
                                 ...
                                 Pathway = c(rep("RTKs", 8),
                                             ...
                                             rep("CDK", 2),
                                             rep("MEK/ERK", 5),
                                             rep("HIF", length(hif1_targets_vec)+1),
                                             rep("Angiotensin\nreceptor", 2)))
...
plot_data_mat2 <- plot_data_mat[(rowSums(!is.na(plot_data_mat)) >= num_nonna),]
## make row ids
