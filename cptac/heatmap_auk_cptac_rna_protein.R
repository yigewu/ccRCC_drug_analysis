...
row_fontsize <- 9
## set genes to plot
genes2plot <- c("AURKA", "AURKB", "AURKC")
...
p <- Heatmap(mat2plot[(rowSums(!is.na(mat2plot)) >= num_row_nonna),],
             column_title = "Aurora kinases mRNA Expressin in CPTAC ccRCC Cohort",
             col = col_rna,
             name = "log10(RPKM+1)", 
...
file2write <- paste0(dir_out, "Aurora_kinases_mRNA_Expression_log10RPKM.", run_id, ".png")
...
## plot heatmaps
p <- Heatmap(mat2plot[(rowSums(!is.na(mat2plot)) >= num_row_nonna),],
             column_title = "Aurora kinases Protein Expressin in CPTAC ccRCC Cohort",
             col = col_protein,
             name = "log2Intensity\n(Sample-Reference)", 
             row_names_gp = gpar(fontsize = 9),
             top_annotation = ca,
             ...
             cluster_rows = F)
## save plot for RNA expression
file2write <- paste0(dir_out, "Aurora_kinases_Protein_Expression_log10RPKM.", run_id, ".png")
png(file2write, width = 2000, height = 250, res = 150)
print(p)
dev.off()
