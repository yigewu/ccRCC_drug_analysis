...
gene_plot_df <- data.frame(gene_symbol = c("Flt1", "Kdr", "MET", "EPHA3", "EGFR", "EGFR", "PDGFRB", "IGF1R", "IGF1R", "IGF1R", "Csf1r", "FGFR1"))
...
# for (i in c(1)) {
  ...
  p <- p + theme(axis.text.x = element_text(size = 12), axis.line.x = element_line(arrow = grid::arrow(length = unit(0.3, "cm"))))
  p <- p + ggtitle(label = paste0(gene_plot, " bulk protein level", ifelse(grepl(pattern = "mouse", plotdata_wide_df$PG.ProteinNames, ignore.case = T), "  (Mouse)", " (Human)")))
