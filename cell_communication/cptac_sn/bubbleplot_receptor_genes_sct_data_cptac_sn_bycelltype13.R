...
source("./ccRCC_drug_analysis/plotting_variables.R")
...
exp_wide_df <- fread(data.table = F, input = "../ccRCC_snRNA/Resources/Analysis_Results/average_expression/avgexp_sct_data_bycelltype13_katmai/20210506.v1/33_aliquot_merged.avgexp.SCT.data.bycelltype13.20210506.v1.tsv")
...
p <- p + guides(fill = guide_legend(override.aes = list(size=3), nrow = 4, byrow = T))
p <- p + ggtitle(label = paste0("Human ccRCC sn Expression"))
file2write <- paste0(dir_out, "legend", ".png")
