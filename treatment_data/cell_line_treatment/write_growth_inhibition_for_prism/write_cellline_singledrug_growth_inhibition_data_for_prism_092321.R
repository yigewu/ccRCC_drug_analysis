...
# input dependencies ------------------------------------------------------
gi_group_sup_df <- fread(data.table = F, input = "./Resources/Analysis_Results/treatment_data/cell_line_treatment/extract_growth_inhibition/extract_cellline_growth_inhibtion_bysingledrugs_092321/20210924.v1/RCC_Cellline.Cabo_Sapa_TM.Single.Growth_Inhibition.20210924.v1.tsv")
...
for (drug_tmp in unique(gi_group_sup_df$Drug[!is.na(gi_group_sup_df$Drug)])) {
  ...
  for (cellline_tmp in unique(gi_group_sup_df$cellline)) {
    ...
    gi_merged_tmp2_df <- gi_merged_tmp_df %>%
      ...
      mutate(conc_unit = ifelse(grepl(pattern = "µm", x = conc_value), "µm",
                                ifelse(grepl(pattern = "nm", x = conc_value), "nm", str_split_fixed(string = conc_value, pattern = " ", n = 2)[,2]))) %>%
      mutate(conc_value = gsub(x = conc_value, pattern = " |nm|µm", replacement = ""))
