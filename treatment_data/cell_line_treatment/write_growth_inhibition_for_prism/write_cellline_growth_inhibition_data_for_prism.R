...
# input dependencies ------------------------------------------------------
gi_group_sup_df <- fread(data.table = F, input = "./Resources/Analysis_Results/treatment_data/extract_cellline_growth_inhibtion_bydrugs/20210819.v1/RCC_Cellline.Growth_Inhibition.20210819.v1.tsv")

# write by drug -----------------------------------------------------------
drug_tmp <- "MK571"
cellline_tmp <- "Hk2"

for (drug_tmp in unique(gi_group_sup_df$Drug)) {
  ...
  for (cellline_tmp in unique(gi_group_sup_df$cellline)) {
    ...
    gi_wide_tmp_df <- dcast(data = gi_merged_tmp_df, formula = conc_value ~ repname, value.var = "growth_inhibition")
