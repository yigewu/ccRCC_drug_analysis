...
drug_conc_map_df <- read_xlsx(path = "./ccRCC_Alamar_Blue/treatment paper alamar blue/concentrations used-rep1 treatmnet paper-Nataly-20210923.xlsx", range = "B3:G6",  col_names = T)
...
drug_conc_long_df <- melt(data = drug_conc_map_df, id.vars = c("Drug", "Targt"))
drug_conc_long_df <- drug_conc_long_df %>%
  select(-Targt) %>%
  mutate(conc_token = tolower(gsub(pattern = " ", replacement = "", x = variable))) %>%
  rename(conc_value = value) %>%
  mutate(drug_token = ifelse(Drug == "Cabozantinib", "Cabo",
                             ifelse(Drug == "Sapanisertib", "Sap",
                                    ifelse(Drug == "Tetrathiomolybdate", "TM", Drug)))) %>%
  select(-variable)
...
# write output ------------------------------------------------------------
file2write <- paste0(dir_out, "RCC_Cellline.", "Cabo_Sapa_TM.", "Single.", "Growth_Inhibition.", run_id, ".tsv")
