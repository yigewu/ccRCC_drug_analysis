...
drug_conc_map_df <- read_xlsx(path = "./ccRCC_Alamar_Blue/ccRCC20210815/ccRCC drug conc plan 08092021.xlsx", range = "A3:H8", sheet = "drug conc", col_names = T)
...
for (filename_tmp in filenames_excel) {
  ...
  gi_df <- read_xlsx(path = path_tmp, range = "B81:K86", sheet = "raw data", col_names = F)
  gi_vec <- as.vector(t(gi_df))
  group_df <- read_xlsx(path = path_tmp, range = "B89:K94", sheet = "raw data", col_names = F)
