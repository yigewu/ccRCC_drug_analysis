...
# make plot data ----------------------------------------------------------
treatment_plot <- "Sapanisertib"
plot_data_long_df <- avg_relative_volume_df %>%
  filter(Treatment == treatment_plot) %>%
...
p
file2write <- paste0(dir_out, treatment_plot, ".complete.", "png")
png(filename = file2write, width = 1000, height = 600, res = 150)
print(p)
dev.off()

pdf.options(reset = TRUE, onefile = FALSE)
file2write <- paste0(dir_out, treatment_plot, ".complete.", "pdf")
...
p <- p + ylab("Tumor volume change (%)") + xlab("Treatment time (days)")
p
file2write <- paste0(dir_out, treatment_plot, ".xlimit50.", "png")
png(filename = file2write, width = 1000, height = 600, res = 150)
print(p)
dev.off()

pdf.options(reset = TRUE, onefile = FALSE)
file2write <- paste0(dir_out, treatment_plot, ".xlimit50.", "pdf")
...
p <- p + ylab("Tumor volume change (%)") + xlab("Treatment time (days)")
p
file2write <- paste0(dir_out, treatment_plot, ".xlimit30.", "png")
png(filename = file2write, width = 1000, height = 600, res = 150)
print(p)
dev.off()

pdf.options(reset = TRUE, onefile = FALSE)
file2write <- paste0(dir_out, treatment_plot, ".xlimit30.", "pdf")
pdf(file2write, width = 5, height = 3, useDingbats = F)
print(p)
dev.off()
