...
avg_relative_volume_df <- fread(data.table = F, input = "./Resources/Analysis_Results/treatment_data/tumor_volume/process_tumor_volume/20210128.v1/RESL10_B1_20210128/RESL10_B1_20210128.RelativeTumoreVolume.Average.ByTreatmentGroup.20210128.v1.tsv")

 # make plot data ----------------------------------------------------------
avg_relative_volume_df <- avg_relative_volume_df %>%
  mutate(Date = as.Date(Date, "%Y-%m-%d")) %>%
  filter(grepl(x = Treatment_Status, pattern = "Treatment on"))
...
p <- p + scale_x_continuous(expand = c(0,0), breaks = seq(0, 56, 7), limits = c(0, 56))
...
p <- p + geom_point()
p <- p + scale_color_manual(values = c("CT" = color_grey, "Cab" = color_red, "Sap" = color_green, "Cab+Sap" = color_yellow))
p <- p + coord_cartesian(ylim = c(-70, 100), xlim = c(0, 70), expand = FALSE)
p <- p + scale_y_continuous(breaks = seq(-70, 100, 10))
p <- p + scale_x_continuous(expand = c(0,0), breaks = seq(0, 56, 7), limits = c(0, 56))
p <- p + theme_classic(base_size = 16)
p <- p + geom_hline(aes(yintercept = 0), linetype = 2)
p <- p + theme(panel.grid.major.y = element_line(colour = "grey90"))
...
p <- p + geom_point()
p <- p + scale_color_manual(values = c("CT" = color_grey, "Cab" = color_red, "Sap" = color_green, "Cab+Sap" = color_yellow))
p <- p + coord_cartesian(ylim = c(100, 700), xlim = c(0, 70), expand = FALSE)
