...
avg_relative_volume_df <- fread(data.table = F, input = "./Resources/Analysis_Results/treatment_data/tumor_volume/process_tumor_volume/20210128.v1/")
...
p <- p + scale_x_continuous(expand = c(0,0), breaks = seq(0, 70, 10))
p <- p + theme_classic(base_size = 16)
p <- p + geom_hline(aes(yintercept = 0), linetype = 2)
p <- p + theme(panel.grid.major.y = element_line(colour = "grey90"),
