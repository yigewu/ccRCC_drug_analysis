...
# input dependencies ------------------------------------------------------
## input genes tested
result_merged_df <- fread(data.table = F, input = "./Resources/Analysis_Results/expression/rna/test/ttest_paired_diff_rna_1month_combo_vs_single/20220322.v1/mRNA.Ttest.Paired.1month.Combo_vs_single.20220322.v1.tsv")
## input genes-to-entrez id mapping
genesymbol2entrezid_df <- fread(data.table = F, input = "./Resources/Analysis_Results/expression/rna/pathway/map_human_mRNA_combo_vs_single_entrezids/20220809.v1/genesymbol2entrezid.tsv")
## input MsigDB 
...
enricher_out_all_df <- NULL
# process -----------------------------------------------------------------
for (diff_direction_tmp in c("down", "up")) {
  genes_test_df <- genes_background_df %>%
    filter(pvalue < cutoff_pvalue) %>%
    filter(diff_direction  == diff_direction_tmp)
  ...
  if (length(enricher_out) > 0 ) {
    ...
    enricher_out_df$diff_direction <- diff_direction_tmp
    enricher_out_all_df <- rbind(enricher_out_all_df, enricher_out_df)
    # # store results
...
# save output -------------------------------------------------------------
## store results
file2write <- paste0(dir_out, "Combo_vs_control.ORA_Results", ".tsv")
