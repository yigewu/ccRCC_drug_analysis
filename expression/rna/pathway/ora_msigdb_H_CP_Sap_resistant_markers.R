...
# input dependencies ------------------------------------------------------
# markers_df <- fread(data.table = F, input = "./Resources/Analysis_Results/expression/rna/get_foldchange/filter_Sap_sensitive_resistant_genes_overlapping_proteins/20211117.v1/Sap_related_genes.20211117.v1.tsv")
markers_df <- fread(data.table = F, input = "./Resources/Analysis_Results/expression/rna/get_foldchange/filter_Sap_sensitive_resistant_genes_overlapping_proteins/20211118.v1/Sap_related_genes.20211118.v1.tsv")
background_genes_df <- fread(data.table = F, input = "./Resources/Analysis_Results/expression/rna/get_foldchange/filter_Sap_sensitive_resistant_genes_overlapping_proteins/20211118.v1/Sap_sensitive_background_genes.20211118.v1.tsv")
## input wikipathway 
...
#                                 mart = ensembl)
genesymbol2entrezid_df <- fread(data.table = F, input = "./Resources/Analysis_Results/expression/rna/pathway/ora_msigdb_H_CP_Sap_resistant_markers/20211117.v1/genesymbol2entrezid.tsv")
...
genes_process_df <- markers_df %>%
  filter(gene_category == "Sap resistant") %>%
...
pdf(file2write, width = 15, height = 6, useDingbats = F)
print(p)
dev.off()
