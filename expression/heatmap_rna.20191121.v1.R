...
## plot a heatmap showing RNA expression for PDX-related samples
...
if (nrow(mut_mat) > 0){
  mut_mat.m <- fread(input = "./Ding_Lab/Projects_Current/RCC/ccRCC_Drug/Resources/Analysis_Results/mutation/make_vaf_table/20191121.v1/ccRCC_PDX_SMG_VAF.20191121.v1.tsv", data.table = F)
  mut_mat.m %>% head()
  mut_mat.m <- data.frame(mut_mat.m)
  colnames(mut_mat.m) <- c("Gene", "New_ID", "Variant_Classification")
  
  ## distinguish by missense and truncation
  mut_mat.m$Variant_Classification[is.na(mut_mat.m$Variant_Classification)] <- ""
  mut_mat.m$Variant_Classification_sim <- "other_mutation"
  mut_mat.m$Variant_Classification_sim[mut_mat.m$Variant_Classification == ""] <- "wild_type"
  mut_mat.m$Variant_Classification_sim[mut_mat.m$Variant_Classification  == "Silent"] <- "silent"
  mut_mat.m$Variant_Classification_sim[grepl(x = mut_mat.m$Variant_Classification, pattern = "Missense_Mutation")] <- "missense"
  mut_mat.m$Variant_Classification_sim[grepl(x = mut_mat.m$Variant_Classification, pattern = "Nonsense_Mutation|Frame_Shift_Ins|Frame_Shift_Del")] <- "truncation"
  mut_mat.m$Variant_Classification_sim[sapply(X = mut_mat.m$Variant_Classification, FUN = function(v) (grepl(pattern = "Nonsense_Mutation|Frame_Shift_Ins|Frame_Shift_Del", x = v) & grepl(pattern = "Missense_Mutation", x = v)))] <- "missense&truncation"
  
  for (gene in unique(mut_mat.m$Gene[mut_mat.m$Variant_Classification_sim != "wild_type"])) {
    mut_mat2merge <- mut_mat.m[mut_mat.m$Gene == gene, c("New_ID", "Variant_Classification_sim")]
...
fn <- paste0(dir_out, paste(geneB, Expression, sep = "_"), ".", run_id, ".png")
