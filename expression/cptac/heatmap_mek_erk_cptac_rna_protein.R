...
p <- Heatmap(mat2plot[(rowSums(!is.na(mat2plot)) >= num_nonna),],
             column_title = "RAS/RAF/MEK/ERK mRNA Expressin in CPTAC ccRCC Cohort",
             col = col_rna,
             name = "log10(RPKM)", 
...
## plot heatmaps
p <- Heatmap(mat2plot[(rowSums(!is.na(mat2plot)) >= num_nonna),],
             column_title = "RAS/RAF/MEK/ERK Protein Expressin in CPTAC ccRCC Cohort",
             col = col_protein,
             name = "log2Intensity\n(Sample-Reference)", 
             row_names_gp = gpar(fontsize = 9),
             top_annotation = ca,
             ...
             cluster_rows = F)
## save plot for RNA expression
file2write <- paste0(dir_out, "RAS_MEK_ERK_Protein_Expression_log10RPKM.", run_id, ".png")
png(file2write, width = 2000, height = 250, res = 150)
print(p)
dev.off()
