...
genes2plot <- c("PIK3CA", "PIK3CB", "PIK3CD", "PIK3R1", "PIK3R2")
...
p <- Heatmap(mat2plot[(rowSums(!is.na(mat2plot)) >= num_row_nonna),],
             column_title = "PI3K_Pathway mRNA Expressin in CPTAC ccRCC Cohort",
             col = col_rna,
             name = "log10(RPKM)", 
...
file2write <- paste0(dir_out, "PI3K_Pathway_mRNA_Expression_log10RPKM.", run_id, ".png")
...
## plot heatmaps
p <- Heatmap(mat2plot[(rowSums(!is.na(mat2plot)) >= num_row_nonna),],
             column_title = "PI3K_Pathway Protein Expressin in CPTAC ccRCC Cohort",
             col = col_protein,
             name = "log2Intensity\n(Sample-Reference)", 
             row_names_gp = gpar(fontsize = 9),
             top_annotation = ca,
             ...
             cluster_rows = F)
## save plot for RNA expression
file2write <- paste0(dir_out, "PI3K_Pathway_Protein_Expression_log10RPKM.", run_id, ".png")
png(file2write, width = 2000, height = 250, res = 150)
print(p)
dev.off()
