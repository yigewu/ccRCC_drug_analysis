...
## input RDS file
srat_integrated <- readRDS(file = path_rds)
DefaultAssay(srat_integrated) <- "RNA"
...
for (id_sample_tmp in unique(srat_integrated@meta.data$orig.ident)) {
  srat <- subset(srat_integrated, idents = id_sample_tmp)
  ## change ident
  srat@meta.data$id_plot <- srat@meta.data$seurat_clusters
  Idents(srat) <- "id_plot"
  ...
  file2write <- paste0(dir_out, id_sample_tmp, ".dotplot.", "scaled.", "png")
  ...
  file2write <- paste0(dir_out, id_sample_tmp, ".dotplot.", "not_scaled.", "png")
  png(filename = file2write, width = 3000, height = 1000, res = 150)
  print(p)
  dev.off()
  cat("Finished writing png!\n\n\n")
