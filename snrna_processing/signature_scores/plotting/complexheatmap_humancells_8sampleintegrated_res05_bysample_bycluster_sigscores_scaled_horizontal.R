...
plotdata_long_df <- results_df %>%
  ...
  mutate(sample = str_split_fixed(string = group, pattern = "_", n = 2)[,1]) %>%
...
plotdata_long_df$treatment[plotdata_long_df$treatment == "CT2"] <- "CT"
plotdata_long_df$treatment[plotdata_long_df$treatment == "Cabo2"] <- "Cab"
plotdata_long_df$treatment[plotdata_long_df$treatment == "Sap2"] <- "Sap"
plotdata_long_df$treatment[plotdata_long_df$treatment == "Cabo_Sap2"] <- "CabSap"
plotdata_long_df <- plotdata_long_df %>%
  mutate(x_plot = gene_set) %>%
  mutate(y_plot = paste0(model, "_", treatment, "_", cluster))
rm(results_df)
