...
## input degs
deg_df <- fread(data.table = F, input = "./Resources/Analysis_Results/snrna_processing/findmarkers/examine_degs/filter_model_specific_degs_humancells_w_bulk_RNA_protein/20210303.v1/Model_Specific_DEGs.20210303.v1.tsv")

# convert gene symbol to entrez ids ---------------------------------------
## filter
deg_df <- deg_df %>%
  filter(!is.na(log2FC) & !is.na(log2Intensity_FC_RESL10vs5))
...
if (length(ora_up_tmp) > 0 ) {
  ...
  p <- p + labs(color = "RESL10 vs 5\nAverage Expression\nFold Change")
  file2write <- paste0(dir_out, "RESL10_vs_RESL5.", "show5", ".png")
  png(filename = file2write, width = 1200, height = 1000, res = 150)
  print(p)
  dev.off()
  
  p <- cnetplot(x = ora_up_tmp, foldChange=de_genes, showCategory = 8)
  p <- p + scale_color_gradient2(low = "blue", high = "red", mid = "white", midpoint = 0)
  p <- p + labs(color = "RESL10 vs 5\nAverage Expression\nFold Change")
  file2write <- paste0(dir_out, "RESL10_vs_RESL5.", "show8", ".png")
