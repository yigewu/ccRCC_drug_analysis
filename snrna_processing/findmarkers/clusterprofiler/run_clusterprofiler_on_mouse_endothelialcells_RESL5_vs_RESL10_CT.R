...
wp2gene <- read.gmt("./Resources/Knowledge/Databases/Wikipathways/wikipathways-20210210-gmt-Mus_musculus.gmt")
wp2gene <- wp2gene %>% tidyr::separate(ont, c("name","version","wpid","org"), "%")
wpid2gene <- wp2gene %>% dplyr::select(wpid, gene) #TERM2GENE
wpid2name <- wp2gene %>% dplyr::select(wpid, name) #TERM2NAME
## input degs
deg_df <- fread(data.table = F, input = "./Resources/Analysis_Results/snrna_processing/findmarkers/examine_degs/filter_model_specific_mouse_endothelialcells_degs/20210302.v1/MouseEndothelialCells.CT.Model_Specific_DEGs.20210302.v1.tsv")

# convert gene symbol to entrez ids ---------------------------------------
## filter
genes2convert <- unique(deg_df$deg_gene_symbol)
## retrieve entrezgene_id
genesymbol2entrezid_df <- getBM(attributes=c('entrezgene_id', 'mgi_symbol'), 
                                filters = 'mgi_symbol', 
                                values = genes2convert, 
                                mart = ensembl)
## add entrez ids to the deg table
deg_df$entrezgene_id <- mapvalues(x = deg_df$deg_gene_symbol, from = genesymbol2entrezid_df$mgi_symbol, to = as.vector(genesymbol2entrezid_df$entrezgene_id))
...
if (length(ora_up_tmp) > 0 ) {
  ...
  ora_up_tmp <- setReadable(ora_up_tmp, org.Mm.eg.db, keyType = "ENTREZID")
  ## store results
  ...
  file2write <- paste0(dir_out, "RESL5_vs_RESL10_Mouse_Endo", ".png")
