...
## input degs
deg_df <- fread(data.table = F, input = "./Resources/Analysis_Results/snrna_processing/findmarkers/findallmarkers_bymanualcluster_humanrcells_8sample_integration_on_katmai/20210212.v1/FindAllMarkers.Wilcox.ByCluster.20210212.v1.tsv")
...
for (deg_group_tmp in unique(deg_df$DEG_Group)) {
  ...
  if (length(ora_up_tmp) > 0 ) {
    ...
    ## store results
    ovaresult_list[[deg_group_tmp]] <- ora_up_tmp
    ...
    file2write <- paste0(dir_out, deg_group_tmp, ".png")
