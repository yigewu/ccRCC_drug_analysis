...
deg_filtered_df <- deg_df %>%
  ...
  mutate(DEG_Group = manual_cluster) %>%
  mutate(avg_logFC = (avg_logFC.x + avg_logFC.y)/2)
nrow(deg_df)

## initiate result 
ovaresult_df <- NULL
ovaresult_list <- list()
for (deg_group_tmp in unique(deg_filtered_df$DEG_Group)) {
  deg_tmp_df <- deg_filtered_df %>%
