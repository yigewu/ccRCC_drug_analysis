...
deg_bulkpro_df <- fread(data.table = F, input = "./Resources/Analysis_Results/expression/protein/other/get_RESL10_vs_RESL5_avg_protein_diff/20210303.v1/RESL10_vs_RESL5.Protein.20210303.v1.tsv")
...
deg_merged_df <- merge(x = deg_merged_df,
                       y = deg_bulkpro_df, by.x = c("deg_gene_symbol", "RESL10_vs_RESL5"), by.y = c("PG.Gene", "RESL10_vs_RESL5.protein"), all.x = T)
...
nrow(deg_merged_df)
nrow(deg_merged_df[!is.na(deg_merged_df$log2FC),])
nrow(deg_merged_df[!is.na(deg_merged_df$log2Intensity_FC_RESL10vs5) & !is.na(deg_merged_df$log2FC),])
nrow(deg_merged_df[!is.na(deg_merged_df$log2Intensity_FC_RESL10vs5) & !is.na(deg_merged_df$log2FC) & deg_merged_df$RESL10_vs_RESL5 == "Up",])
nrow(deg_merged_df[!is.na(deg_merged_df$log2Intensity_FC_RESL10vs5) & !is.na(deg_merged_df$log2FC) & deg_merged_df$RESL10_vs_RESL5 == "Down",])

# write output ------------------------------------------------------------
file2write <- paste0(dir_out, "Model_Specific_DEGs.", run_id, ".tsv")
