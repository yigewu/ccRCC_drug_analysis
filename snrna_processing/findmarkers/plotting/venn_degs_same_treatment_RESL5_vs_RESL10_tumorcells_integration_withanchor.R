...
deg_filtered_df2 <- deg_filtered_df %>%
  ...
  select(Model_ID, Treatment_Group1, sampleid_group1, Species_Feature, deg_gene_symbol, FoldChange_Direction, avg_logFC, p_val_adj, pct.1, pct.2, p_val)
## overlap
deg_filtered_overlap_df <- merge(deg_filtered_df1, deg_filtered_df2, by = c("Treatment_Group1", "Species_Feature", "deg_gene_symbol", "FoldChange_Direction"), all = T)
# write output ------------------------------------------------------------
...
for (treatment_group1 in unique(deg_filtered_overlap_df$Treatment_Group1)) {
  plot_data_df <- deg_filtered_overlap_df %>%
    filter(Treatment_Group1 == treatment_group1) %>%
    mutate(RESL5 = !is.na(Model_ID.x)) %>%
    mutate(RESL10 = !is.na(Model_ID.y))
  fit <- euler(combinations = plot_data_df[,c("RESL5", "RESL10")], input = "disjoint", shape = 'circle')
  p <-plot(fit, quantities = list(fontsize = 30), legend = list(fontsize = 50))
  ## save output
  file2write <- paste0(dir_out, treatment_group1, "_vs_CT.", "RESL5_vs_RESL10", ".png")
