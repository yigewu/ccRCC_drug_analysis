...
print(sampleids)
sampleids_group1 <- sampleids[grepl(x = sampleids, pattern = "RESL10")]
## spcify assay
assay_process <- "RNA"
...
for (sampleid_group1 in sampleids_group1) {
  ...
  if (treatment_name == "Cabo_Sap") {
    sampleid_group2 <- sampleids[grepl(x = sampleids, pattern = "RESL5") & grepl(x = sampleids, pattern = treatment_name)]
  } else {
    sampleid_group2 <- sampleids[grepl(x = sampleids, pattern = "RESL5") & grepl(x = sampleids, pattern = treatment_name) & !grepl(x = sampleids, pattern = "Cabo_Sap")]
  }
...
# write output ------------------------------------------------------------
file2write <- paste0(dir_out, "FindMarkers.", "Wilcox.", ".RESL10_vs_RESL5.","tsv")
