...
fetchdata_df <- fread(data.table = F, input = "./Resources/Analysis_Results/snrna_processing/map_barcode/map_barcode2idmanualcluster_RESL_8sample_tumorcells_integration_withanchor/20200508.v1/barcode2idmanualcluster_umapdata.20200508.v1.tsv")
...
p <- p + ggtitle(label = paste0("Human ", " Cell Integration"), subtitle = paste0("CT & Cab & Sap & (Cab+Sap)"))
p
## wreite output
...
## make plot
p <- ggplot()
p <- p + geom_point(data = plotdata_df, mapping = aes(x = UMAP_1, y = UMAP_2, color = Id_Seurat_Cluster), size = 0.2, alpha = 0.8)
