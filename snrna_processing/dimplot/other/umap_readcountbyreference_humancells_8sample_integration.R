...
# specify parameters ------------------------------------------------------
## make colors
colors_mouse <- c("white", RColorBrewer::brewer.pal(n = 9, name = "YlOrRd"))
colors_mouse_vals <- c(0, 20, 40, 60, 80, 100, 1000, 2000, 3000, 4000)/4000
...
ref_tmp <- "mm10"
plotdata_df[, "count_plot"] <- plotdata_df[, ref_tmp]
plotdata_df <- plotdata_df %>%
  arrange(count_plot)
## make plot
p <- ggplot()
p <- p + geom_point(data = plotdata_df, mapping = aes(x = UMAP_1, y = UMAP_2, color = count_plot), 
                    size = 0.2, alpha = 0.8)
# p <- p + scale_color_manual(values = colors_cluster)
p <- p + scale_colour_gradientn(colours = colors_mouse, values = colors_mouse_vals) # 
