...
plotdata_df <- barcode2cluster_df %>%
  mutate(Id_Cluster = factor(Id_Manual_Cluster))
## make color for each cluster
colors_cluster <- Polychrome::dark.colors(n = length(unique(plotdata_df$Id_Cluster)))
names(colors_cluster) <- unique(plotdata_df$Id_Cluster)
## make plot
p <- ggplot()
...
for (cluster_tmp in unique(barcode2cluster_df$seurat_clusters)) {
  ## make plot data
  plotdata_df <- barcode2cluster_df %>%
    mutate(Id_Cluster = ifelse(seurat_clusters == cluster_tmp, cluster_tmp, "others"))
  ## make color for each cluster
