...
barcode2cluster_df <- fread(input = "./Resources/Analysis_Results/snrna_processing/clustering/cluster_RESL_tumorcells_integration_withanchor_on_katmai/20200507.v1/umap_data.RESL.Tumor_cells.integration.withanchor.20200507.v1.PC40.Res0.5.tsv", data.table = F)
...
for (cluster_tmp in unique(barcode2cluster_df$seurat_clusters)) {
  ...
  p
  ## write output
  file2write <- paste0(dir_out1, "umap.", "clusters", cluster_tmp, ".", "allsamples.", id_integration, ".png")
  png(filename = file2write, width = 1200, height = 1000, res = 150)
  print(p)
  dev.off()
}
