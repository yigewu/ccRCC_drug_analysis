...
for (sample_id in path_qm_df$sample_id) {
  ...
  qm_filtered_df <- qm_df %>%
    ...
    filter(call != "Multiplet") %>%
    filter(mitoRatio <= 0.1) %>%
    filter(nCount_RNA >= 1000 & nCount_RNA <= 80000) %>%
    filter(nFeature_RNA >= 200 & nFeature_RNA <= 10000)
  
  ## make scatterplot plot for number of UMI counts per species per barcode
  plot_data_df <- qm_filtered_df %>%
  ...
  p <- p + ggtitle(label = paste0(sample_id, ": ", nrow(plot_data_df), " Seurat-Filtered Barcodes"),
